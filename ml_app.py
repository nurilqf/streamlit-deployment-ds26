...
## ambil values dari masing-masing variable di atas
def get_value(val,my_dict):
    for key, value in my_dict.items():
        if val == key:
            return value
        
## memanggil dan menyimpan model dalam variable yang udah ditentuin
@st.cache
...
def run_ml_app():
    ...
    prediction_probability = model.predict_proba(single_sample)

    st.write(prediction)
    st.write(prediction_probability)

    if prediction == 1:
        st.write("Congratulation, you got promotion from us")
    elif prediction == 0:
        st.write("You need to work harder than before")
